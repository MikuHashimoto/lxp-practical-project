            ...
            <h1>{{ $product->name }}</h1>
                <div class="product-price">
                    <span class="product-price-text">
                        {{ $product->price * config('cart.jpy_rate')}}
                    </span>
                    <span class="product-price-symbol">
                        {{config('cart.currency_symbol')}}
                    </span>
                    <span class="product-cost">
                        ＋送料{{config('cart.shipping_cost')}}{{config('cart.currency_symbol')}}
                    </span>
                </div>
            <p>SKU:{{ $product->sku}}</p>
                    ...
                    <form action="{{ route('cart.store') }}" class="form-inline" method="post">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control" name="quantity" id="quantity"
                                placeholder="数量を入力" value="{{ old('quantity') }}" />
                            <input type="hidden" name="product" value="{{ $product->id }}" />
                        </div>
                        <button type="submit" class="add-cart btn btn-warning">
